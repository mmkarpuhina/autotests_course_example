...
def everything_for_your_cat(cats_data):
    """
функция оптимизирует список покупателей и их котиков
    :param cats_data: данные о кошках и их покупателях в виде кортежа
    :return:выводим список покупатель - все его котики, с новой строки каждый покупатель
    """
    # Здесь нужно написать код
    cats_dict = {}
    for a in range(len(cats_data)):
        fio = ' '.join(cats_data[a][2:])
        cat = ', '.join(map(str, cats_data[a][:2]))
        cats_dict[fio] = f'{cats_dict[fio]}; {cat}' if cats_dict.get(fio) else cat

    our_str = ''
    for a in range(len(cats_dict)):
        our_str = f"{': '.join(cats_dict.popitem())}\n{our_str}"
