...
def repeats(our_str):
    """
    функция считает, сколько раз символ встречается в строке
    :param our_str: заданная строка
    :return: выводит количество каждого символа в заданной строке через постфикс
    """
    # Здесь нужно написать код
    letters_dict = {}
    new_str = ''
    for symbol in our_str:
        if letters_dict.get(symbol) is None:
            letters_dict.update({symbol: 1})
            new_str = new_str + f'{symbol}_1'
        else:
            amount = letters_dict.get(symbol) + 1
            letters_dict.update({symbol: amount})
            new_str = new_str + f'{symbol}_{amount}'
    return new_str
