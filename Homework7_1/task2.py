...
class PersonInfo:
    def __init__(self, name, age, *department):
        """
        Метод инициализации класса PersonInfo
        :param name: Имя Фамилия сотрудника
        :param age: Возраст сотрудника
        :param department: Подразделения от головного до того, где работает сотрудник
        """
        self.name = name
        self.age = age
    ...
    def short_name(self):
        """
        Функция возвращает строку Фамилия Имя.
        :return: Фамилия Имя сотрудника.
        """
    ...
    def path_deps(self):
        """
        Функция возвращает путь от головного подразделения до того, где работает сотрудник.
    ...
    def new_salary(self):
        """
        Функция вычисляет зарплату по формуле:
        1337*Возраст*суммарное кол-во вхождений трех наиболее часто встречающихся букв
        из списка подразделений
        :return: Зарплата суммой
        """
        self.department = ''.join(self.department)
        summa = {item: self.department.count(item) for item in self.department}
        summa = sorted(summa.values())
        salary = 1337 * self.age * sum(summa[-3:])
        return salary
